...
const MAXIMIZER_STRING = "item +equip thor's pliers +equip kol con snowglobe +equip lucky gold ring +equip cheeng +equip screege +equip ittah bittah hookah -back -hat"

function maybeBjorn(f: Familiar) {
  if(myBjornedFamiliar() != f) {
    bjornifyFamiliar(f);
  }
}

function maybeEnthrone(f: Familiar) {
  if(myEnthronedFamiliar() != f) {
    enthroneFamiliar(f); 
  }
}
...
function pickDrumMachineFamiliar() {
  ...
  let relevantBuffs = myEffects.filter(([effect, turns]:[Effect, number]) => ["Item Drop", "Meat Drop", "Monster Level"].some((modifier) => numericModifier(effect, modifier) > 0));

  let [minEffect, minTurns] = relevantBuffs.reduce(
    ([aggEffect, aggTurns]:[Effect, number], [curEffect, curTurns]:[Effect, number]) => aggTurns > curTurns ? [curEffect, curTurns] : [aggEffect, aggTurns]
  )

  print(`Minimum effect ${minEffect} has ${minTurns} turns`)
...
class DrunkPygmy {
  static freeBanishes() {
    return get('questL11Worship') !== 'unstarted' && (get('_drunkPygmyBanishes') < 10 || (get('_drunkPygmyBanishes') == 10 && have($item`miniature crystal ball `)))
  }

  static shouldSaber() {
    return get('questL11Worship') !== 'unstarted' && 
    ((get('_drunkPygmyBanishes') == 10 && !have($item`miniature crystal ball`)) 
    || (get('_drunkPygmyBanishes') == 11 && have($item`miniature crystal ball`) && getProperty("crystalBallMonster") == "drunk pygmy"));
  }
  ...
  static didSaber() {
    return get('questL11Worship') !== 'unstarted' && getProperty('_saberForceMonster') === "drunk pygmy" && get('_saberForceMonsterCount') > 0;
  }
}
...
class CosplaySaber {
  ...
  static canGive(mode: SaberUpgrade) {
    ...
    return have($item`Fourth of May Cosplay Saber`) && (currentUpgrade === SaberUpgrade.Unupgraded || currentUpgrade == mode); 
  }
...
class FreeRun {
  ...
  static maybeMacro() {
    return Macro
            .externalIf(equippedAmount($item`navel ring of navel gazing`) + equippedAmount($item`greatest american pants`) > 0 && get('_navelRunaways') < 3, Macro.step("runaway"))
            .externalIf(availableAmount($item`peppermint parasol`) > 0 && get('_navelRunaways') < 3, Macro.item($item`peppermint parasol`))
            ...
            .externalIf(equippedAmount($item`V for Vivala Mask`) > 0 && !get('_vmaskBanisherUsed'), Macro.skill($skill`Creepy Grin`))
            .externalIf(equippedAmount($item`stinky cheese eye`) > 0 && !get('_stinkyCheeseBanisherUsed'), Macro.skill($skill`Give Your Opponent the Stinkeye`))
            .externalIf(equippedAmount($item`Lil' Doctor™ bag`) > 0 && get('_reflexHammerUsed') < 3, Macro.skill($skill`Reflex Hammer`))
            .externalIf(equippedAmount($item`Kremlin's Greatest Briefcase`) > 0 && get('_kgbTranquilizerDartUses') < 3, Macro.skill($skill`KGB tranquilizer dart`))
            .externalIf(equippedAmount($item`mafia middle finger ring`) > 0 && !get('_mafiaMiddleFingerRingUsed'), Macro.skill($skill`Show them your ring`))
            .externalIf(myFamiliar() == $familiar`nanorhino` && get("_nanorhinoCharge") == 100, Macro.externalIf(!have($effect`Nanobrawny`), Macro.skill($skill`shell up`)).skill($skill`Unleash the Nanites`))
            .abort()
  }
...
step("fax", () => !get('_photocopyUsed'), () => faxbot(FREE_FIGHT_COPY_TARGET, "Cheesefax"))(
  () => {
    withMacro(
      Macro.tentacle()
...
step(
  "spooky putty", 
  () => SpookyPutty.hasFight()
)(() => {
  withMacro(
    Macro
      .tentacle(Macro.externalIf(get("_feelNostalgicUsed") < 3, Macro.skill($skill`Feel Nostalgic`)))
...
step(
  ...
  () => setChoices(new Map([[1322, 2], [1324, 5]]))
)(() => {
...
step(
  ...
  FreeRun.wrapFreeRun(() => {
    adventureMacro(
      $location`Menagerie Level 1`, 
      Macro.tentacle().kramco().step(FreeRun.maybeMacro()).abort())
  });
});
...
step(
  ...
  () => get("_banderRunaways") < maxFamiliarRuns() || (CosplaySaber.canGive(SaberUpgrade.FamiliarWeight) && equippedAmount($item`May the Fourth Cosplay Saber`) === 0 && get("_banderRunaways") == maxFamiliarRuns()),
  () => {
  ...
  if(get("_banderRunaways") == maxFamiliarRuns() && equippedAmount($item`May the Fourth Cosplay Saber`) == 0 && CosplaySaber.canGive(SaberUpgrade.FamiliarWeight)) {
  ...
  adventureMacro(
    ...
    Macro.tentacle()
      ...
      .externalIf(get('_mayflySummons') < 30, Macro.if_("monstername basic elemental", Macro.trySkill($skill`Summon Mayfly Swarm`).abort()))
      .step("runaway")
  );
});
...
step(
  "final asdon martin", 
  () => !get("_missileLauncherUsed") && getCampground()['Asdon Martin keyfob'] !== undefined || !get("_workshedItemUsed") && get("_"), 
  () => { 
...
step(
  ...
  let nextFood = $items``.find((i) => effectModifier(i, "effect") == $effect`Thanksgetting` && thanksgetting.indexOf(i) == -1)
  if(nextFood) {
    eat(nextFood);
  } else {
...
step(
  "smooch soda", 
  () => mallPrice($item`SMOOCH soda`) < 1000 && getRemainingStomach() > 0 || (MayoClinic.present() && getRemainingLiver() > 0))
(() => {
