import { availableAmount, equip, familiarWeight, maximize, print, retrieveItem, useFamiliar, visitUrl, weightAdjustment } from "kolmafia";
...
export const getSewersState = memoizeTurncount(() => {
  ...
  export function getSewerItems() {
    return !$items`unfortunate dumplings, sewer wad, bottle of ooze-o, gatorskin umbrella`.some((i) => !retrieveItem(1, i)) && retrieveItem(3, $item`oil of oiliness`);
  }
  ...
  export function main() {
    ...
    while(!throughSewers()) {
      ...
      adventureMacro($location`A Maze of Sewer Tunnels`, 
